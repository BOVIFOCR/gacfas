...
class SiWDataset(Dataset):    
    def __init__(self, 
        ...
        self.video_list = (get_video_list(pathlib.Path(root_dir)/"train")
                            + get_video_list(pathlib.Path(root_dir)/"test"))
    ...
    def is_live(self, video_path):
        try:
            return str(video_path).split('-')[2] == '1'
        except IndexError:
            raise IndexError(str(video_path))
